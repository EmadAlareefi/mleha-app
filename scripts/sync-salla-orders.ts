...
function printUsage(): void {
  console.log(`
Salla Orders Sync CLI
=====================

Fetches orders directly from https://api.salla.dev/admin/v2/orders using the stored tokens
inside the SallaAuth table and saves them locally.

Usage:
  npm run sync:salla-orders -- [options]
...
Examples:
  ...
  npm run sync:salla-orders -- --merchant 123456 --start-date 2024-01-01 --end-date 2024-03-31
`.trim());
}
...
function summarizeStatus(stats: Awaited<ReturnType<typeof syncSallaOrders>>): ExitStatus {
  const failedMerchants = stats.filter(
    (stat) => stat.pagesProcessed === 0 && stat.ordersFetched === 0 && stat.errors.length > 0
  );
...
async function main() {
  try {
    ...
    const stats = await syncSallaOrders({
    ...
    for (const stat of stats) {
      ...
      console.log(`Orders fetched  : ${stat.ordersFetched}`);
      console.log(`Orders stored   : ${stat.ordersStored}`);
      if (stat.errors.length > 0) {
        console.log(`Errors (${stat.errors.length}):`);
        for (const err of stat.errors.slice(0, 5)) {
          console.log(`  - [Order ${err.orderId ?? 'n/a'}] ${err.message}`);
        }
    ...
    console.error('Order sync failed:', error instanceof Error ? error.message : error);
    process.exitCode = 1;
  } finally {
